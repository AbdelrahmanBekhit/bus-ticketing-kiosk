...
export default function App() {
  ...
  else if (path.startsWith('/trip-prompt')) progress = 100
  else if (path.startsWith('/route-confirmation')) progress = 100
  else if (path.startsWith('/ticket-options')) progress = 100
  else if (path.startsWith('/ride-info')) progress = 100
  else if (path.startsWith('/route-pricing')) progress = 100
  else if (path.startsWith('/monthly-pass-upsell')) progress = 100
    ...
    <LangCtx.Provider value={value}>
      ...
      <Modal open={showTimeout} onClose={() => setShowTimeout(false)}>
        <div className="text-center space-y-2">
          <p className="font-semibold">{t.sessionTimeoutWarning}</p>
          <button onClick={() => setShowTimeout(false)} className="btn">{t.stillHere}</button>
        </div>
      </Modal>
    </LangCtx.Provider>
  )
